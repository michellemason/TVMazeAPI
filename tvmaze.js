...
async function getEpisodes(id) {
  ...
  let episode = episodes.data.map(function(res) {
    return {
      id: res.id,
      name: res.name,
      season: res.season,
...
function populateEpisodes(episodeArr) {
  ...
  for (let episode of episodeArr) {
    let $item2 = $(
      `<li>${episode.name} (season ${episode.season}, episode ${episode.number})</li>
      `);

    $episodeList.append($item2);
  }
  $("#episodes-area").show();
}

$("#shows-list").on('click', '#epBtn', async function handleEpClick(e) {
